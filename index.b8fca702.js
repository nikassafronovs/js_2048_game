...
class Game {
    ...
    moveLeft() {
        ...
        for(let r = 0; r < 4; r++)this.board[r] = this.moveRowLeft(this.board[r]);
        if (JSON.stringify(this.board) !== oldBoard) this.addRandomTile();
        this.checkWin();
        this.checkLose();
        this.render();
    }
   ...
   */ restart() {
    ...
    addRandomTile(isStart = false) {
        ...
        if (isStart) this.board[random.r][random.c] = 2;
        else this.board[random.r][random.c] = Math.random() < 0.9 ? 2 : 4;
    }
    ...
    checkWin() {
        for (const row of this.board)if (row.includes(2048)) this.status = "win";
    }
    checkLose() {
